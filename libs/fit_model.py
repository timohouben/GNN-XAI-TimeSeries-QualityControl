...
def train_model(model, model_config, preproc_config, train_dataset_batched, val_dataset_batched=None, 
                baseline=False, classes_weights=None, labels=None):
    ...
    if model_config.graph_convolution.layer!='GATConv':
        mc = ModelCheckpoint('../model_checkpoint', mode='min', save_best_only=True)
    else:
        mc = ModelCheckpoint('../model_checkpoint', monitor='val_loss', mode='min', save_best_only=True, save_weights_only=True)
    callback_list = [PlotLossesKeras(), es, mc]
